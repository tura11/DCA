...
contract DCA is AutomationCompatibleInterface {
    ...
    function setPosition(uint256 amount, uint256 period) external {
        ...
        Position memory position =
            Position({amount: amount, period: period, lastExecuted: block.timestamp, active: true});
    ...
    function _withdraw(address user) internal {
        ...
        if (block.timestamp - positions[user].lastExecuted < positions[user].period) {
            revert DCA__YouCannotWithdrawYet();
        }
    ...
    function updatePositionAmount(uint256 newAmount) external {
        ...
        Position memory position =
            Position({amount: newAmount, period: positonCurrentPeriod, lastExecuted: block.timestamp, active: true});
